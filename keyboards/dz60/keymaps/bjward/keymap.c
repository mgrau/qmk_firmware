...
#define RT_CTL MT(MOD_RCTL, KC_RGHT)
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    KC_LCTL, KC_LGUI, KC_LALT,          LOW_MOD,          KC_SPC,           RAS_MOD,          MO(FUN), KC_LEFT, KC_DOWN, KC_UP,   RT_CTL  \
    ),
	[RAISE] = MGLAYOUT(
...
uint32_t layer_state_set_user(uint32_t state) {
  return update_tri_layer_state(state, LOWER, RAISE, ADJ);
}
